...
В настоящее время локальное API поддерживает скрипты `post`, `follow` и `unfollow`. Поддержка других скриптов будет добавлена в будущих версиях.
:::
...
| `account_warmup` | Разогрев аккаунта | 🔜 Скоро |
